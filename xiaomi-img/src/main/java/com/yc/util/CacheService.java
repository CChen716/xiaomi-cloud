...
public class CacheService {
     ...
     *
     */
    public <K,  V> void addHashCacheAll(K key, Map<K,V> value, long timeout, TimeUnit unit ) {
        redisTemplate.opsForHash().putAll(DEFAULT_KEY_PREFIX+key,value);
        redisTemplate.expire((String) key,timeout,unit);
    }
    ...
    public <K, SK, V> void addHashCache(K key, SK subKey, V value, long timeout, TimeUnit unit) {
        redisTemplate.opsForHash().put(DEFAULT_KEY_PREFIX + key, subKey, value);   //添加
        redisTemplate.expire(DEFAULT_KEY_PREFIX + key, timeout, unit);       //设置过期时间
    }
